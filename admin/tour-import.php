...
if (isset($_GET['cancel'])) {
    ...
    header('Location: ' . BASE_URL . '/admin/tour-import.php');
    exit;
}
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <h3 style="font-size:13px;font-weight:600;margin-bottom:8px;">Importálandó túrák (<?= count($preview['rows']) ?> db)</h3>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Dátum</th>
            <th>Típus</th>
            <th style="text-align:right;">Napok</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <td><code style="font-size:.85em;white-space:nowrap;"><?= e($r['tour_code']) ?></code></td>
            <td><?= e($r['name'] ?? '—') ?></td>
            <td><?= e($r['country']) ?><?= $r['region'] ? ' – ' . e($r['region']) : '' ?></td>
            <td><?= $r['tour_date'] ? formatDate($r['tour_date']) : '—' ?></td>
            <td><?= e(getTourTypeLabel($r['tour_type'])) ?></td>
            <td style="text-align:right;"><?= (int)$r['days'] ?></td>
          </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
    ...
    <form method="post" action="<?= BASE_URL ?>/actions/tour-import.php">
      <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
      <input type="hidden" name="step" value="confirm">
      <div class="flex gap-2">
        <button type="submit" class="btn btn-primary">
          Importálás jóváhagyása (<?= count($preview['rows']) ?> túra)
        </button>
        <a href="<?= BASE_URL ?>/admin/tour-import.php?cancel=1" class="btn btn-secondary">Mégse</a>
      </div>
    </form>

    <?php else: ?>
    <p style="color:var(--text-muted);margin-bottom:16px;">Nincs importálható sor. Ellenőrizd a CSV fájlt és próbáld újra.</p>
    <a href="<?= BASE_URL ?>/admin/tour-import.php?cancel=1" class="btn btn-secondary">← Új fájl feltöltése</a>
    <?php endif; ?>
