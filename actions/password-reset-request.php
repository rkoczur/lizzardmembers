...
$user = $pdo->prepare("SELECT id, firstname, lastname, email, active FROM users WHERE email = ? LIMIT 1");
$user->execute([$email]);
$user = $user->fetch();
...
// Send email
$resetUrl  = BASE_URL . '/password-reset.php?token=' . rawurlencode($token);
...
$html      = '<!DOCTYPE html><html><body style="font-family:sans-serif;max-width:520px;margin:40px auto;color:#222;">'
           ...
           . 'style="background:#1a4a46;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:600;">Jelszó visszaállítása</a></p>'
           . '<p style="font-size:12px;color:#888;">Ha nem Ön kérte, hagyja figyelmen kívül ezt az e-mailt. A link 1 óra után automatikusan érvénytelenné válik.</p>'
...
try {
    ...
    $mailer->send($user['email'], $name, APP_NAME . ' – Jelszó visszaállítása', $html);
} catch (Throwable $e) {
    // Log silently, show generic message to user
