#!/usr/bin/env python3
"""Download bird images from Wikimedia/Wikipedia for birds.json entries."""
...
USER_AGENT = "KvizBirdImageDownloader/1.0 (https://github.com/local/kviz; educational quiz project)"
THUMB_SIZE = 800
RATE_LIMIT_SEC = 1.0
...
def resolve_search_terms(bird: dict) -> list[str]:
    terms: list[str] = []
    if bird.get("latin_name"):
        terms.append(bird["latin_name"])
    name = bird["name"]
    if BINOMIAL_RE.match(name) and name not in terms:
        terms.append(name)
...
def find_image(bird: dict) -> tuple[dict | None, list[str]]:
    tried: list[str] = []
    terms = resolve_search_terms(bird)

    for term in terms:
        tried.append(f"hu.wikipedia pageimages:{term}")
        result = page_image("https://hu.wikipedia.org/w/api.php", term)
        if result:
            return result, tried

    hungarian_name = bird["name"]
    tried.append(f"hu.wikipedia search:{hungarian_name}")
    result = wiki_search_then_image("https://hu.wikipedia.org/w/api.php", hungarian_name)
    if result:
        return result, tried

    for term in terms:
        tried.append(f"en.wikipedia pageimages:{term}")
        result = page_image("https://en.wikipedia.org/w/api.php", term)
        if result:
            return result, tried
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download bird images from Wikimedia.")
    ...
    parser.add_argument("--limit", type=int, default=0, help="Process only the first N birds.")
    args = parser.parse_args()

    root = Path(__file__).resolve().parent.parent
    birds_path = root / "birds.json"
    images_dir = root / "images"
    report_path = root / "download_report.json"
    attribution_path = root / "image_attribution.json"

    all_birds = json.loads(birds_path.read_text(encoding="utf-8"))
    birds = all_birds[: args.limit] if args.limit > 0 else all_birds
    ...
    for bird in birds:
        ...
        image_info, tried = find_image(bird)
        ...
        rel_path = f"images/{unique_image_path(images_dir, slug, image_info['url'], used_names).name}"
        ...
        updated += 1
        print(f"[ok] {name} -> {bird['image']}")

    if not args.dry_run:
        if args.limit > 0:
            by_name = {b["name"]: b for b in birds}
            for bird in all_birds:
                if bird["name"] in by_name:
                    if "image" in by_name[bird["name"]]:
                        bird["image"] = by_name[bird["name"]]["image"]
        birds_path.write_text(
            json.dumps(all_birds, ensure_ascii=False, indent=4) + "\n",
            encoding="utf-8",
        )
        ...
        if attribution:
            ...
            if attribution_path.exists():
                existing_attr = json.loads(attribution_path.read_text(encoding="utf-8"))
            existing_attr.update(attribution)
            attribution_path.write_text(
                json.dumps(existing_attr, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
    else:
        report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
